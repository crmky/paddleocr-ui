...
from paddleocr_ui.utils import (
    escape_inequalities_in_math,
    file_to_base64,
...
def call_api(
    ...
    is_url = isinstance(path_or_url, str) and path_or_url.startswith(
        ("http://", "https://")
    )
    ...
    if restructure_pages:
        ...
        payload["mergeTables"] = merge_tables
        payload["relevelTitles"] = relevel_titles
    ...
    if settings.debug:
        ...
        if "file" in log_payload and isinstance(log_payload["file"], str):
            ...
            if len(file_data) > 200:
                log_payload["file"] = (
                    f"{file_data[:100]}...({len(file_data)} chars)...{file_data[-50:]}"
                )
            ...
            for placeholder_path, image_data in md_images_map.items():
                if isinstance(image_data, str):
                    image_src = (
                        ...
                        else process_base64_image(image_data)
                    )
                    md_text = md_text.replace(
                        f'src="{placeholder_path}"', f'src="{image_src}"'
                    ).replace(f"]({placeholder_path})", f"]({image_src})")
        ...
        for key in sorted(out_imgs.keys()):
            ...
            if img_data and isinstance(img_data, str):
                all_images.append(
                    ...
                    else process_base64_image(img_data)
                )
    ...
    if all_images:
        ...
        for idx, img_src in enumerate(all_images):
            if len(layout_results) > 1:
                vis_parts.append(
                    f'<p style="text-align:center;color:#64748b;margin:8px 0;">Page {idx + 1}</p>'
                )
            vis_parts.append(
                f'<img src="{img_src}" alt="Page {idx + 1}" loading="lazy" style="max-width:100%;margin-bottom:16px;">'
            )
        output_html = "\n".join(vis_parts)
    else:
        output_html = (
